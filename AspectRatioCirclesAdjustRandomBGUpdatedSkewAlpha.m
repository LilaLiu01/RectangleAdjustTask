...
while (vbl < (blockStart+blockLen)) && ~done
    ...
    while ~matchSet && ~done && (vbl < (blockStart+blockLen))
        ...
        bigRect = GrowRect(theRect,bgOvalPad,bgOvalPad);
        
        Screen('FillRect',offw, [0 0 0 0], winRect);
        ...
        Screen('FillOval',offw, [fudgemean fudgemean fudgemean 255],bigRect);

        if  (t < grayEndTime)
            Screen('DrawTexture', w, offw,bigRect,bigRect,45);
        elseif (t < circleEndTime)          
            if rord == 0
                Screen('FrameOval',offw, c1, theRect,frameWidth);
            elseif rord == 1
                Screen('FillOval',offw, c1, theRect);
            elseif rord == 2
                Screen('FrameOval',offw, c1, theRect,2*frameWidth);
            end
            Screen('DrawTexture', w, offw, bigRect,bigRect, 45);
