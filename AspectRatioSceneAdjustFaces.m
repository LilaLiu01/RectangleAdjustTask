...
%Get list of human faces from Chicago Face Database
directory = 'cfd_main';
cfd = dir('cfd_main/*.jpg');
nfaces = length(cfd);
...
while (vbl < (blockStart+blockLen)) && ~done
    ...
    %random human face image for testing
    facenum = randi(nfaces);
    fname = cfd(facenum).name;
    testImg = imread(fullfile(directory, fname));
    ...
    while ~matchSet && ~done && (vbl < (blockStart+blockLen))
        ...
        kernel = mkGaussKernel([testSize testSize],[testSize/4 testSize/4]);
        kernel = kernel./max(max(kernel));
